...
from .dc_ae import ARMS, POSTHOC_ARMS, DIMS

INK, MUTED, GRIDC = "#0b0b0b", "#52514e", "#e4e3df"
AC = {"RECON": "#2a78d6", "JAC-OPP": "#eb6834", "G-MSE": "#1baf7a", "REGRET": "#4a3aa7", "OBS-RECON": "#e87ba4"}
AM = {"RECON": "o", "JAC-OPP": "s", "G-MSE": "^", "REGRET": "D", "OBS-RECON": "v"}
...
def main():
    ...
    AR = ARMS + [a for a in POSTHOC_ARMS if f"{a}_d2" in tb]; LB = lambda a: a + (" (post-hoc)" if a in POSTHOC_ARMS else "")
    ...
    for a in AR:
        ...
        axs[0].plot(x, y, color=AC[a], marker=AM[a], lw=2, label=LB(a), ls="--" if a in POSTHOC_ARMS else "-"); axs[0].fill_between(x, lo, hi, color=AC[a], alpha=0.10, lw=0)
    ...
    for a in AR:
        ...
        ax.plot(bx, by, color=AC[a], marker=AM[a], lw=1.8, ms=6, label=f"{LB(a)} (autoencoder, d = 2…64)", ls="--" if a in POSTHOC_ARMS else "-")
    ...
    fig, axs = plt.subplots(1, 2, figsize=(14, 4.6)); w = 0.8 / len(AR)
    for i, a in enumerate(AR):
        axs[0].bar(np.arange(3) + (i - (len(AR) - 1) / 2) * w, [tb[f"{a}_d8"][f"test_frac_{e}"] for e in EPS_LIST], width=w * 0.92, color=AC[a], label=LB(a))
        vals = [tb[f"{a}_d8"]["test_frac_0.1"]] + [tb[f"{a}_d8"][f"ood_frac_{f}"] for f in OOD_FAMS]
        axs[1].bar(np.arange(5) + (i - (len(AR) - 1) / 2) * w, vals, width=w * 0.92, color=AC[a], label=LB(a))
