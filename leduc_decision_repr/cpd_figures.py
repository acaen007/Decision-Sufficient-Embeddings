...
STY = {"PRIOR-EM": ("#008300", "*", "-", 2.2, "full history"), "PRIOR-EM-WIN": ("#1baf7a", "s", "-", 2.2, "window 50"),
       ...
       "CPD-PRIOR-EM": ("#2a78d6", "o", "-", 2.6, "change-detect + reset"), "KNOWN-CUSUM-RESET": ("#8a8984", "D", ":", 1.8, "oracle detector (knows q_A, q_B)"),
...
def main():
    r = json.loads((D / "analysis.json").read_text()); MS = list(STY)
    ...
    order = ["POST-PRIOR-EM", "KNOWN-CUSUM-RESET", "CPD-PRIOR-EM", "PRIOR-EM-WIN", "DISC-PRIOR-EM g=0.98", "DISC-PRIOR-EM g=0.95", "PRIOR-EM"]
    ...
    det = r["detection"]; grp = np.repeat(np.array(json.loads(json.dumps(r["SWITCH"]["contrasting"]["original_grid"]["n_processes"])) * [0]), 1)
    fig, axs = plt.subplots(1, 3, figsize=(15, 4.6))
    ax = axs[0]
    P = np.load(D / "processes.npz", allow_pickle=True); hsel = np.repeat(P["SW_sel"], 2)
    for name, c, ls in (("CPD", STY["CPD-PRIOR-EM"][0], "-"), ("KNOWN-CUSUM", STY["KNOWN-CUSUM-RESET"][0], ":")):
        for g, lw in (("contrasting", 2.2), ("random", 1.1)):
            ...
            ax.step(xs, ys, where="post", color=c, ls=ls, lw=lw, label=f"{'CPD (practical)' if name == 'CPD' else 'known-model CUSUM (oracle detector)'}, {g}")
    if "floor" in r:
    ...
    ax = axs[1]
    for name, c in (("CPD", STY["CPD-PRIOR-EM"][0]), ("KNOWN-CUSUM", STY["KNOWN-CUSUM-RESET"][0])):
        pass
    ...
    ax.annotate("calibrated choice", (cost[m], min(rt, 6)), xytext=(10, 10), textcoords="offset points", fontsize=8, color=INK)
    for yv in (1.5, 2.0):
    ...
    ax.set_title("hindsight: all 45 configurations on test (marker size ∝ τ)", fontsize=9.5, color=INK); ax.legend(fontsize=7.5, frameon=False, loc="upper right")
    ...
    for m in dr["fraction"]:
        line(ax, DR_T, dr["fraction"][m], dr["fraction_ci"][m][0], dr["fraction_ci"][m][1], m, band=m in ("CPD-PRIOR-EM", "PRIOR-EM-WIN"))
